...
while [ "true" ];do 
    ...
    if [ -s /tmp/cloudflared.out ]; then
        cloudflared_url_new=$(flock -s  /tmp/cloudflared.out -c "cat /tmp/cloudflared.out" | grep -oE "https://[a-zA-Z0-9.-]+\.trycloudflare\.com");
...
if [ -s /tmp/cloudflared-ssh.out ]; then
    cloudflared_ssh_url_new=$(flock -s  /tmp/cloudflared-ssh.out -c "cat /tmp/cloudflared-ssh.out" | grep -oE "https://[a-zA-Z0-9.-]+\.trycloudflare\.com");
...
if [ -s /tmp/cloudflared-ttyd.out ]; then
    cloudflared_ttyd_url_new=$(flock -s  /tmp/cloudflared-ttyd.out -c "cat /tmp/cloudflared-ttyd.out" | grep -oE "https://[a-zA-Z0-9.-]+\.trycloudflare\.com");
